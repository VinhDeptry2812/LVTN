...
const OrderHistoryTab: React.FC<OrderHistoryTabProps> = ({ user }) => {
  ...
  return (
    <div className="bg-white border border-outline-variant/40 rounded-sm p-6 md:p-8 shadow-[0_4px_20px_rgba(0,0,0,0.02)] space-y-6 animate-fadeIn">
      ...
      {/* Modal Hủy Đơn Hàng */}
      {cancelModalOrderId !== null && createPortal(
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm transition-all duration-300">
          <div className="bg-white rounded-sm w-full max-w-md shadow-2xl flex flex-col border border-outline-variant/30 overflow-hidden">
            <div className="p-5 border-b border-outline-variant/20 flex items-center justify-between bg-surface-container-low">
              ...
              <button
                ...
                className="text-on-surface-variant hover:text-on-surface p-1 rounded-sm transition"
              >
                ✕
              </button>
            </div>
            ...
            <div className="p-4 border-t border-outline-variant/20 bg-surface-container-low flex justify-end gap-2.5">
              <button
                type="button"
                onClick={() => setCancelModalOrderId(null)}
                className="px-4 py-2 text-xs font-semibold text-on-surface-variant hover:bg-surface-container rounded transition"
              ...
              <button
                ...
                disabled={isSubmittingCancel}
                className="px-4 py-2 text-xs font-bold text-white bg-error hover:bg-error/90 rounded transition shadow-sm disabled:opacity-50 flex items-center gap-1.5"
              >
