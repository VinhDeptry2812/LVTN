...
export class AuthService {
  ...
  async googleLogin(req) {
    ...
    if (!user) {
      ...
      const salt = await bcrypt.genSalt(10);
      const password_hash = await bcrypt.hash('google_' + Date.now() + Math.random().toString(), salt);
  ...
  async logout() {
    return { message: 'Đăng xuất thành công. Vui lòng xóa Token ở phía Client.' };
  }
  ...
  async forgotPassword(email: string) {
    ...
    if (!user) {
      ...
      return { message: 'Nếu email hợp lệ, một mã xác nhận sẽ được gửi tới email của bạn.' };
    }
